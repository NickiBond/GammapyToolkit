...
def update_spectral_fields(saved_args):
    ...
    for model in models:
        ...
        for param_label, param_key in spectral_model_parameters[model]:
            ...
            key = f"{model}_{param_key}"
            spectral_entries[key] = (label, entry)

            # ✨ Insert saved or default value
            value = saved_args.get(key, DEFAULT_ARGS.get(key, ""))
...
model_entry.bind("<FocusOut>", update_spectral_fields)
model_entry.bind("<Return>", update_spectral_fields)
