...
def MakeLightCurve(path_to_log, datasets, args):
    ...
    lc.to_table(format="lightcurve", sed_type="flux")["time_min", "time_max", "flux", "flux_err"].write(
        args.ADir + "/LightCurve/LightCurve.ecsv", overwrite=True)
    f.write("Light Curve Flux Points saved to " + args.ADir + "/LightCurve/LightCurve.ecsv\n")
    f.write("To open: \n")
    f.write("from astropy.table import Table \n")
    f.write(f"LC = Table.read({args.ADir}/LightCurve/LightCurve.ecsv, format='ascii.ecsv') \n")
    f.write("--------------------------------------------------\n")
    f.close()
    """
